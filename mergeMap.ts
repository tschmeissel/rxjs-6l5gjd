import { Observable, fromEvent, interval, of, throwError } from 'rxjs';
import {
  switchMap,
  mergeMap,
  delay,
 ...
 * Each run does an async and subscribe, and emit stream of responses.
 ...
 * collects all individual observables and returns all observables in a single array without caring about the order of observable
 * works asynchronously.
 * The inner observable has to emit a value to make mergeMap merge that one with the outer
 * observable. So the inner observable starts the merge process.
